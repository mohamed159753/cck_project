...
export class ProfRevComponent {
  ...
  setMinStartTime() {
    const now = new Date();
    this.minStartTime = this.formatDateTimeLocal(now);
  }

  onImmediatelyChange() {
    if (this.startImmediately) {
      console.log(this.startImmediately)
      const now = new Date();
  ...
  onStartTimeChange() {
    if (this.startTime) {
      ...
      this.minEndTime = this.formatDateTimeLocal(minEndDate);

      if (this.endTime) {
        const endDate = new Date(this.endTime);
        // If endTime is earlier than minEndTime, update it
  ...
  // Add this new method to handle end time changes
  onEndTimeChange() {
    if (this.endTime && this.startTime) {
      ...
      if (endDate <= startDate) {
        const minEndDate = new Date(startDate.getTime() + 60 * 60 * 1000); // 1 hour later
        this.endTime = this.formatDateTimeLocal(minEndDate);
      }
    }
  }

  // Enhanced method to validate datetime inputs
  validateDateTimeInputs(): boolean {
    ...
    // Check if start time is in the past (unless starting immediately)
    if (!this.startImmediately && startDate < now) {
    ...
    if (endDate.getTime() - startDate.getTime() < minDuration) {
      this.errorMessage = 'Minimum reservation duration is 1 hour.';
      return false;
    }
  ...
  onSubmitRequest() {
    ...
    // Validate datetime inputs first
